...
import { FaBath, FaShareAlt, FaBed, FaUserCircle, FaPlus,  FaMapMarkedAlt, FaCamera, FaVideo, FaLaptop, FaHeart } from 'react-icons/fa';
  ...
  return (
    ...
    <div className={`${darkMode ? 'dark: bg-black' : 'light: bg-transparent'}`}>
      <section id='properties' className='lg:w-[90%] m-auto lg:px-20 px-6 py-20 w-full flex flex-col justify-center items-start gap-10'>
        ...
        <div className=' flex flex-col justify bg-center items-start gap-4'>
          <h1 data-aos='zoom-out' className='text-red-500  dark:text-white '>PROPERTIES</h1>
          <h1 data-aos='zoom-in' className='text-4xl font-semibold dark: text-slate-900 dark:text-white'>Explore the latest</h1>
        </div>
        ...
        <div id='grid-box' className='w-full grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8'>

          {
            property.map((item, index) => (
              <div data-aos="zoom-in" data-aos-delay="400" key={index} className='bg-white dark:bg-gray-800 rounded-xl w-full' >
                <div id='image-box' className='bg-cover bg-center h-[250px] rounded-lg  p-4 flex flex-col justify-between items-end' style={{ backgroundImage:`url(${item.images})`}}>
                  ...
                  <div id='bottom' className='justify-between flex items-end w-full'>
                    <div className='flex justify-start items-center gap-2'>
                      < FaMapMarkedAlt className='size-6 text-white' />
                      <h1 className='text-white'>{item.address}</h1>
                    </div>
                    <div className='flex justify-center items-center gap-4'>
                      < FaVideo className='size-6 text-white'/>
                ...
                <div className='px-6 py-3 flex flex-col justify-center items-start gap-2 w-full'>
                  ...
                  <div id='owner-info' className='flex justify-between items-center w-full mt-2'>
                      <div className='flex justify-center items-center gap-2'>
                        <FaUserCircle className='size-6 text-red-500'/>
                        <h1 className='text-sm text-black font-semibold dark:text-white'>{item.owner}</h1>
                      </div>

                      <div className='flex justify-center items-center gap-4'>
                        <div className='p-2 border-2 border-gray-200 hover:bg-black cursor-pointer transform hover:scale-110 transition-transform duration-300'><FaShareAlt className='size-4 text-red-400'/>
                        </div>
                        <div className='p-2 border-2 border-gray-200 hover:bg-black cursor-pointer transform hover:scale-110 transition-transform duration-300'><FaHeart className='size-4 text-red-400'/>
                        </div>
                        <div className='p-2 border-2 border-gray-200 hover:bg-black cursor-pointer transform hover:scale-110 transition-transform duration-300'><FaPlus className='size-4 text-red-400'/>
                        </div>
